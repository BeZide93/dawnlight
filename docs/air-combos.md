...
Positive vertical velocity keeps ordinary gravity. Once descending, gravity is
-0.08 and terminal fall speed is -0.8 game units per tick. Horizontal sword lunge
speed is capped at six units per tick. The native position, wall, floor and roof
collision pipeline still runs; there is no position teleport or ground-hit spoof.
The whole string has a 90-execution budget (three seconds at 30 Hz), shared across
all its swings. No swing adds lift or resets the budget. Early landing or an
unbuffered combo end can end it sooner.
...
buffered inputs and restores native gravity. `procFallInit(0, ...)` preserves
...
airtime across reinitialization, unchanged fall height/ascent, landing, toggles,
