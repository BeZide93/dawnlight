...
struct daAlink_c: fopAc_ac_c {
 enum daAlink_PROC {PROC_WAIT,PROC_MOVE,PROC_CROUCH,PROC_AUTO_JUMP,PROC_FALL,PROC_DAMAGE};
 enum {MODE_SWIMMING=1,ANM_JUMP_LAND=1,UPPER_1=1};
 daAlink_PROC mProcID=PROC_WAIT;
 ...
 void setGrabItemPos(){} void procWaitInit(){mProcID=PROC_WAIT;}
 bool procCrouchInit(){if(!initSucceeds)return false;mProcID=PROC_CROUCH;return true;}
 bool procAutoJumpInit(int){if(!initSucceeds)return false;++launches;mProcID=PROC_AUTO_JUMP;speed.y=25;return true;}
};
...
bool s_deferredJumpTrigger=false;daAlink_c* s_manualJumpOwner=nullptr;
int active_jump_binding(){return 0;}bool jump_pressed(int){return pressed||s_deferredJumpTrigger;}bool jump_held(int){return held;}
...
// HEIGHT
bool start_ground_jump(daAlink_c* l){
 if(!rjump||!jump_pressed(0)||!ground_jump_context_ready(l))return false;
 if(!l->procAutoJumpInit(1))return false;
 apply_manual_jump_movement(l);apply_manual_jump_height(l,jump_height_multiplier());return true;
}
// ABILITIES
...
void charge(daAlink_c& l){gale=true;assert(tick(l,true,true));for(int i=0;i<59;++i)assert(tick(l,false,true));assert(s_jumpAbilities.charge==GaleCharge::Holding);assert(tick(l,false,true));assert(s_jumpAbilities.charge==GaleCharge::Ready);assert(l.mProcID==daAlink_c::PROC_CROUCH);}
int main(){
 daAlink_c l;
 ...
 setup(l);gale=true;l.input=true;l.mProcID=daAlink_c::PROC_MOVE;assert(!tick(l,true,true));
 setup(l);gale=true;assert(tick(l,true,true));assert(l.launches==0);assert(tick(l,false,false));assert(l.launches==1);close(l.speed.y,25);
 setup(l);charge(l);assert(!handle_jump_abilities(&l)); // No double charge tick in one execute.
 assert(tick(l,false,false));assert(l.launches==1);close(l.speed.y,50);assert(!l.mLinkAcch.ground);assert(s_manualJumpOwner==&l);
 setup(l);heightPercent=500;charge(l);assert(tick(l,false,false));close(l.speed.y,25*std::sqrt(20.0f));
 setup(l);rjump=false;charge(l);assert(tick(l,false,false)); // Gale independent from R Jump.
 setup(l);gale=true;rjump=false;assert(tick(l,true,true));assert(!tick(l,false,false));assert(l.launches==0);
 for(int scenario=0;scenario<7;++scenario){
  setup(l);charge(l);
  switch(scenario){case 0:gale=false;break;case 1:l.input=true;break;case 2:event=true;break;case 3:stage=true;break;case 4:l.wolf=true;break;case 5:l.action=true;break;case 6:l.mLinkAcch.ground=false;break;}
  assert(!tick(l,false,false));assert(s_jumpAbilities.charge==GaleCharge::Idle);assert(l.launches==0);assert(l.mProcID==daAlink_c::PROC_WAIT);
 }
